...
class TargetController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $currentValue = $target ? max($target->current_value, $todayRevenue) : $todayRevenue;
        $progress     = $targetValue > 0 ? min(100, round(($currentValue / $targetValue) * 100, 1)) : 0;
        $remaining    = max(0, $targetValue - $currentValue);
        $lastUpdated  = now()->diffForHumans(short: false);
        $avgHarian    = $this->avgDailyRevenue(30);

        $hoursElapsed  = max(1, now()->diffInHours(today()->setTime(8, 0)));
        $estimasi      = (int) round(($currentValue / $hoursElapsed) * 14);
        ...
        $staffPerformance = Transaction::query()
            ...
            ->select('cashier_id', DB::raw('SUM(total_amount) as total'))
            ->groupBy('cashier_id')
            ->orderByDesc('total')
            ->limit(3)
            ->with('cashier')
        ...
        $paymentSummary = $paymentMethods->map(
            fn ($count, $method) => strtoupper($method) . ' (' . round(($count / $totalTrx) * 100) . '%)'
        )->implode(', ');
        ...
        $peakLabel  = $peakHour
            ? sprintf('%02d:00 - %02d:00', $peakHour->hour, $peakHour->hour + 2)
            : '12:00 - 14:00';

        $promoAktif = '-';

        return view('shared.targets-goals.index', compact(
            'target', 'targetValue', 'currentValue', 'progress',
            'remaining', 'lastUpdated', 'period',
            'estimasi', 'trendEstimasi', 'avgHarian',
            'history', 'staffPerformance', 'maxStaff',
            'paymentSummary', 'peakLabel', 'promoAktif',
            'today',
        ));
    }
