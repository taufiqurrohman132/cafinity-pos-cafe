...
@section('content')
    <div class="space-y-6 p-4 md:p-6 bg-[#fbfbfe] min-h-screen relative">
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center justify-between">
            <div>
                <h1 class="text-[28px] font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[#050316] to-[#2f27ce] tracking-tight">Dashboard Owner</h1>
            ...
            <div class="flex flex-wrap items-center gap-3">
                <div class="text-[13px] text-[#2f27ce] bg-white px-4 py-2.5 rounded-xl border border-[#dddbff] shadow-sm flex items-center gap-2 font-medium">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-6">
                ...
                <div class="bg-white p-6 rounded-2xl border border-[#dddbff] shadow-sm">
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6">
                        <div>
                            ...
                            <p class="text-xs text-[#2f27ce] font-medium">Tren pendapatan hari ini, {{ $todayLabel }}</p>
                        </div>
                    ...
                    <div class="flex items-center gap-2 mb-6">
                        <button class="px-4 py-1.5 text-xs font-bold rounded-lg border transition-all bg-[#2f27ce] text-white border-[#2f27ce] shadow-sm">Hari Ini</button>
                        <button class="px-4 py-1.5 text-xs font-bold rounded-lg border transition-all bg-white text-[#2f27ce] border-[#dddbff] hover:bg-[#dddbff]/50 hover:text-[#050316]">7 Hari</button>
                        <button class="px-4 py-1.5 text-xs font-bold rounded-lg border transition-all bg-white text-[#2f27ce] border-[#dddbff] hover:bg-[#dddbff]/50 hover:text-[#050316]">30 Hari</button>
                        <button class="px-4 py-1.5 text-xs font-bold rounded-lg border transition-all bg-white text-[#2f27ce] border-[#dddbff] hover:bg-[#dddbff]/50 hover:text-[#050316]">Bulan Ini</button>
                    </div>
                            ...
                            <div class="flex-1 flex flex-col items-center gap-1 group cursor-pointer"
                                ...
                                @php
                                    $tooltip = $point['amount'] > 0 
                                        ...
                                        : 'Rp 0';
                                @endphp
                                <span class="text-[10px] font-bold text-white opacity-0 group-hover:opacity-100 transition-opacity bg-[#050316] px-2 py-0.5 rounded-md mb-1 shadow-md">{{ $tooltip }}</span>
                                <div class="w-full bg-[#443dff] rounded-t-md transition-all duration-300 group-hover:bg-[#2f27ce] min-h-[4px] shadow-sm group-hover:shadow-[#443dff]/30"
                                    style="height: {{ max($point['height'], 4) }}%"></div>
                                <span class="text-[10px] font-bold text-[#2f27ce]/50 group-hover:text-[#050316] transition-colors">{{ $salesChart['labels'][$index] ?? '' }}</span>
                            </div>
                        @empty
                            <p class="w-full text-center text-[#2f27ce] italic text-sm py-16">Belum ada penjualan hari ini</p>
                        @endforelse
                    </div>
                    <div class="flex gap-4 mt-6 text-xs font-bold text-[#2f27ce] justify-center">
                        <span class="flex items-center gap-2"><span class="w-2.5 h-2.5 rounded-full bg-[#443dff] shadow-sm"></span> Pendapatan</span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white p-6 rounded-2xl border border-[#dddbff] shadow-sm">
                        <div class="flex justify-between items-center mb-5">
                            ...
                            <a href="{{ route('menus.index') }}" class="text-xs text-[#443dff] font-extrabold hover:text-[#2f27ce] hover:underline transition-colors">Lihat Katalog</a>
                        </div>
                        <div class="space-y-4">
                            @forelse ($bestSellingMenus as $menu)
                                <div class="flex items-center justify-between p-2.5 hover:bg-[#dddbff]/20 rounded-xl transition-colors">
                                    <div class="flex flex-wrap items-center gap-3">
                                        <div class="w-12 h-12 bg-[#dddbff]/30 border border-[#dddbff] rounded-xl flex items-center justify-center text-2xl shadow-sm">
                                            {{ $menu['emoji'] }}
                                        </div>
                                        <div>
                            ...
                            @empty
                                <p class="text-sm text-[#2f27ce] italic text-center py-4">Belum ada data penjualan menu hari ini.</p>
                            @endforelse
                        </div>
                    </div>
                ...
                <div class="bg-white p-6 rounded-2xl border border-[#dddbff] shadow-sm">
                    <div class="flex justify-between items-center mb-5">
                        ...
                        <a href="{{ route('recipe.index') }}"
                            class="text-xs font-bold border border-[#dddbff] text-[#2f27ce] px-4 py-1.5 rounded-lg hover:bg-[#dddbff] hover:text-[#050316] transition-colors shadow-sm">Detail HPP</a>
                    </div>
                            ...
                            <tbody class="text-sm">
                                @forelse ($profitability as $row)
                                    <tr class="border-b border-[#dddbff]/50 last:border-0 hover:bg-[#dddbff]/10 transition-colors">
                                        ...
                                        <td class="py-4 text-right font-extrabold text-[#050316]">{{ $row['margin'] }}</td>
                                    </tr>
                                @empty
                                    <tr>
            ...
            <div class="xl:col-span-3 space-y-6">
                ...
                <div class="bg-white p-5 rounded-2xl border border-[#dddbff] shadow-sm relative overflow-hidden">
                    <div class="absolute top-0 right-0 w-24 h-24 bg-[#dddbff] rounded-full blur-2xl -mr-10 -mt-10 pointer-events-none"></div>
                    <div class="relative z-10">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-[11px] font-extrabold uppercase tracking-widest text-[#2f27ce]">Goal Hari Ini</h3>
                        ...
                        <div class="w-full bg-[#dddbff]/50 h-2.5 rounded-full overflow-hidden mb-3 shadow-inner">
                            <div class="bg-[#443dff] h-full transition-all duration-500 ease-out" style="width: {{ $dailyGoal['progress'] }}%"></div>
                        </div>
                        <p class="text-[11px] font-medium text-[#2f27ce] leading-relaxed">
                            @if ($dailyGoal['progress'] >= 100)
                                Target <span class="font-extrabold text-[#050316]">{{ $dailyGoal['target'] }}</span> tercapai! 🎉
                            @else
                ...
                <div class="bg-white p-5 rounded-2xl border border-[#dddbff] shadow-sm">
                    <div class="flex justify-between items-center mb-5">
                        ...
                        <span class="bg-rose-100 text-rose-700 text-[10px] px-2.5 py-1 rounded-md font-extrabold border border-rose-200">
                        ...
                        @forelse ($lowStockItems as $item)
                            <a href="{{ route('inventories.show', $item->id) }}"
                                ...
                                <div class="flex justify-between items-center gap-2">
                                    <div>
                                        <p class="text-xs font-bold text-[#050316] group-hover:text-rose-900 transition-colors">
                                            {{ $item->name }}</p>
                ...
                <div class="bg-white p-5 rounded-2xl border border-[#dddbff] shadow-sm">
                    <div class="flex justify-between items-center mb-5">
                        ...
                        <div class="flex items-center gap-1.5">
                            <span class="flex items-center gap-1 text-[10px] font-bold text-[#2f27ce] bg-[#dddbff]/50 px-2 py-0.5 rounded-md"><span class="w-1.5 h-1.5 bg-[#443dff] rounded-full animate-pulse"></span> Live</span>
                            <a href="{{ route('detail.antrean') }}"
                                class="text-[10px] text-[#443dff] font-extrabold hover:text-[#2f27ce] hover:underline ml-1">Lihat semua &rarr;</a>
                        </div>
                    </div>
                    <div class="space-y-3">
                            ...
                            <a href="{{ route('detail.antrean') }}"
                                ...
                                <div class="flex justify-between items-start gap-3 pl-3">
                                    <div class="flex-1 min-w-0">
                                        <p class="text-xs font-extrabold text-[#050316] group-hover:text-[#443dff] transition-colors truncate">
                                        ...
                                        <div class="flex items-center gap-2 mt-1.5">
                                            ...
                                            @else
                                                <span class="text-[9px] font-bold bg-[#dddbff] text-[#2f27ce] px-2 py-0.5 rounded-md border border-[#dddbff]">Menunggu</span>
                                            @endif
                                        </div>
                                    </div>
                                    <span class="text-[#443dff] text-sm opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 translate-x-2 group-hover:translate-x-0 transform duration-200">
                                        &rarr;
                                    </span>
                                </div>
                            </a>
                        @empty
                            <p class="text-xs text-[#2f27ce] italic text-center py-2">Tidak ada pesanan di dapur saat ini.</p>
                        @endforelse
                    </div>
                </div>
                ...
                <a href="{{ route('bundles.index') }}"
                    ...
                    <div class="relative z-10">
                        <p class="text-[10px] font-extrabold text-[#dddbff] mb-1.5 tracking-widest">✨ PROMO AKHIR PEKAN?</p>
                        ...
                        <span class="inline-flex items-center gap-1.5 text-xs font-extrabold text-[#050316] bg-[#fbfbfe] rounded-lg px-4 py-2 transition-all duration-300 group-hover:bg-[#dddbff] shadow-sm">
                            Buat Sekarang <span class="group-hover:translate-x-1 transition-transform text-[#443dff]">&rarr;</span>
                        </span>
                    </div>
    ...
    {{-- ====== SET TARGET MODAL ====== --}}
    <dialog id="goal-modal" class="backdrop:bg-[#050316]/60 backdrop:backdrop-blur-sm backdrop:rounded-none transition-all">
        <div class="bg-white rounded-2xl w-full max-w-lg p-6 shadow-2xl mx-4 sm:mx-auto my-auto border border-[#dddbff]">
            <div class="flex items-center justify-between mb-6 border-b border-[#dddbff]/50 pb-4">
                <div>
                    <h2 class="text-xl font-extrabold text-[#050316] tracking-tight">Set Target Harian</h2>
                    <p class="text-xs font-medium text-[#2f27ce] mt-1">Tentukan target pendapatan untuk hari ini ({{ $today->translatedFormat('d F Y') }})</p>
                </div>
            ...
            <form method="POST" action="{{ route('targets-goals.store') }}" class="space-y-5">
                ...
                <div>
                    <label class="block text-xs font-extrabold text-[#2f27ce] mb-1.5 uppercase tracking-wide">Label Target <span class="text-rose-500">*</span></label>
                    <input type="text" name="label" placeholder="cth: Target Harian Shift Pagi" required
                        class="w-full h-11 rounded-xl border border-[#dddbff] text-sm px-4 focus:outline-none focus:ring-2 focus:ring-[#dddbff] focus:border-[#443dff] transition-shadow bg-[#fbfbfe] focus:bg-white text-[#050316] font-semibold"
                        value="{{ $currentTarget?->label ?? 'Target Pendapatan Harian' }}">
                    <p class="text-[10px] font-medium text-[#2f27ce]/70 mt-1.5">Gunakan label yang jelas (misal: Target Hari Sabtu).</p>
                </div>

                <div>
                    <label class="block text-xs font-extrabold text-[#2f27ce] mb-1.5 uppercase tracking-wide">Target Pendapatan (Rp) <span class="text-rose-500">*</span></label>
                    <div class="relative">
                        ...
                        <input type="number" name="target_value" min="1" step="1000" required
                            ...
                            class="w-full h-11 rounded-xl border border-[#dddbff] text-sm pl-12 pr-4 focus:outline-none focus:ring-2 focus:ring-[#dddbff] focus:border-[#443dff] transition-shadow bg-[#fbfbfe] focus:bg-white text-[#050316] font-extrabold"
                            value="{{ $currentTarget?->target_value ?? '' }}">
                    </div>
                    <p class="text-[10px] font-medium text-[#2f27ce]/70 mt-1.5">Target minimal pendapatan untuk mencapainya hari ini.</p>
                </div>

                <div>
                    <label class="block text-xs font-extrabold text-[#2f27ce] mb-1.5 uppercase tracking-wide">Pendapatan Saat Ini (Rp)</label>
                    <div class="relative">
                        <span class="absolute left-4 top-1/2 -translate-y-1/2 font-bold text-[#2f27ce] text-sm pointer-events-none">Rp</span>
                        <input type="number" name="current_value" min="0" step="1000"
                            placeholder="Terserah — dihitung otomatis"
                            class="w-full h-11 rounded-xl border border-[#dddbff] text-sm pl-12 pr-4 focus:outline-none focus:ring-2 focus:ring-[#dddbff] focus:border-[#443dff] transition-shadow bg-[#fbfbfe] focus:bg-white text-[#050316] font-extrabold placeholder:font-medium placeholder:text-[#2f27ce]/40"
                            value="{{ $currentTarget?->current_value ?? ($dailyGoal['remaining'] ?? 0) }}">
                    </div>
                    <p class="text-[10px] font-medium text-[#2f27ce]/70 mt-1.5">Opsional. Nilai ini diperbarui otomatis dari data penjualan.</p>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-extrabold text-[#2f27ce] mb-1.5 uppercase tracking-wide">Awal Periode</label>
                        <input type="date" name="start_date"
                            value="{{ $currentTarget?->start_date ?? $today->format('Y-m-d') }}"
                            class="w-full h-11 rounded-xl border border-[#dddbff] text-sm px-4 focus:outline-none focus:ring-2 focus:ring-[#dddbff] focus:border-[#443dff] transition-shadow bg-[#fbfbfe] focus:bg-white text-[#050316] font-semibold cursor-pointer">
                    </div>
                    <div>
                        <label class="block text-xs font-extrabold text-[#2f27ce] mb-1.5 uppercase tracking-wide">Akhir Periode</label>
                ...
                <div class="flex items-center gap-3 pt-4 border-t border-[#dddbff]/50 mt-2">
                    <button type="submit"
