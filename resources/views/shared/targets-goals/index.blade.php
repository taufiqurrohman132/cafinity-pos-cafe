...
@section('content')
    <div class="space-y-6 p-4 md:p-6 bg-[#fbfbfe] min-h-screen">
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                @foreach (['harian' => 'Harian', 'mingguan' => 'Mingguan', 'bulanan' => 'Bulanan'] as $val => $label)
                    <a href="{{ route('targets-goals.index', ['period' => $val]) }}"
                        class="px-5 py-2.5 text-xs font-bold rounded-xl border transition-all
                {{ $period === $val ? 'bg-[#2f27ce] text-white border-[#2f27ce] shadow-sm' : 'bg-white text-[#2f27ce] border-[#dddbff] hover:bg-[#dddbff]/50' }}">
                        {{ $label }}
                    </a>
                @endforeach
            </div>
        ...
        <div class="bg-white rounded-2xl border border-[#dddbff] shadow-sm p-6 relative overflow-hidden">
            <div
                class="absolute top-0 right-0 w-64 h-64 bg-[#dddbff]/30 rounded-full blur-3xl -mr-20 -mt-20 pointer-events-none">
            </div>
            <div class="relative z-10">
                ...
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <span class="text-sm font-extrabold text-[#443dff] uppercase tracking-widest">
                        Target {{ ucfirst($period) }}
                    </span>
                    @if ($target?->label)
                        <span
                            class="text-[10px] font-bold text-[#2f27ce]/60 bg-[#dddbff]/50 px-2 py-0.5 rounded-md border border-[#dddbff]">
                            {{ $target->label }}
                        </span>
                    @endif
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                    ...
                    <div class="md:col-span-2 space-y-3">
                        <div class="flex items-end justify-between gap-4">
                            <div>
                                <p class="text-[10px] font-extrabold text-[#2f27ce] uppercase tracking-widest mb-1">Tercapai
                                </p>
                            ...
                            <div class="text-right">
                                <p class="text-[10px] font-extrabold text-[#2f27ce] uppercase tracking-widest mb-1">Progress
                                </p>
                        ...
                        <div class="flex justify-between text-[10px] font-bold text-[#2f27ce]/60">
                            <span>
                                {{ $period === 'harian' ? today()->format('d M Y') : '' }}
                                {{ $period === 'mingguan' ? today()->startOfWeek()->format('d M') . ' – ' . today()->endOfWeek()->format('d M Y') : '' }}
                                {{ $period === 'bulanan' ? today()->format('F Y') : '' }}
                            </span>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div class="bg-white rounded-2xl border border-[#dddbff] shadow-sm p-5">
                <div class="flex items-start justify-between mb-3">
                    <p class="text-[10px] font-extrabold text-[#2f27ce] uppercase tracking-widest leading-tight">Sisa Target
                    </p>
            ...
            <div class="bg-white rounded-2xl border border-[#dddbff] shadow-sm p-5">
                <div class="flex items-start justify-between mb-3">
                    <p class="text-[10px] font-extrabold text-[#2f27ce] uppercase tracking-widest leading-tight">Estimasi
                        Penutupan</p>
                    <div class="w-9 h-9 rounded-xl bg-[#dddbff] flex items-center justify-center flex-shrink-0">
            ...
            <div class="bg-white rounded-2xl border border-[#dddbff] shadow-sm p-5">
                <div class="flex items-start justify-between mb-3">
                    <p class="text-[10px] font-extrabold text-[#2f27ce] uppercase tracking-widest leading-tight">Rata-rata
                        Harian</p>
                    <div class="w-9 h-9 rounded-xl bg-[#dddbff] flex items-center justify-center flex-shrink-0">
            ...
            <div class="bg-white rounded-2xl border border-[#dddbff] shadow-sm p-5">
                <div class="flex items-start justify-between mb-3">
                    <p class="text-[10px] font-extrabold text-[#2f27ce] uppercase tracking-widest leading-tight">Update
                        Terakhir</p>
                    <div class="w-9 h-9 rounded-xl bg-[#dddbff] flex items-center justify-center flex-shrink-0">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-6">
            ...
            <div class="xl:col-span-8 bg-white rounded-2xl border border-[#dddbff] shadow-sm p-6">
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-6">
                    <div>
                        <h3 class="text-base font-extrabold text-[#050316] tracking-tight">Riwayat Pencapaian (30 Hari
                            Terakhir)</h3>
                        <p class="text-xs text-[#2f27ce] font-medium mt-0.5">Perbandingan antara target harian vs realisasi
                            penjualan</p>
                    </div>
                    <div
                        class="flex items-center gap-1.5 text-[10px] font-bold text-[#050316] bg-[#dddbff]/50 border border-[#dddbff] px-3 py-1.5 rounded-lg flex-shrink-0">
                ...
                <div class="flex items-center gap-6 mt-5 text-[11px] font-bold text-[#2f27ce] justify-center">
                    ...
                    <span class="flex items-center gap-2">
                        <span class="w-3 h-0.5 bg-[#050316] rounded-full inline-block"
                            style="border-top: 2px dashed #050316; background:transparent;"></span>
                        Target Penjualan
                    </span>
                </div>
            </div>
                ...
                <div class="bg-white rounded-2xl border border-[#dddbff] shadow-sm p-5">
                    ...
                    <div class="mb-4 pb-4 border-b border-[#dddbff]">
                        ...
                        <p class="text-[11px] font-medium text-[#2f27ce] leading-relaxed">
                            Pukul <span class="font-extrabold text-[#050316]">{{ $peakLabel }}</span> biasanya menjadi
                            jam tersibuk berdasarkan data 7 hari terakhir.
                        </p>
                        @if ($promoAktif !== '-')
                            <div class="mt-2 flex items-center gap-2 flex-wrap">
                                <span
                                    class="text-[10px] font-extrabold text-[#443dff] bg-[#dddbff] px-2 py-0.5 rounded-md border border-[#c4c0ff]">Promo
                                    Aktif</span>
    ...
    {{-- ====== SET TARGET MODAL ====== --}}
    <x-modal.target :current-target="$target" :estimasi="$progress ?? 74.2" />
